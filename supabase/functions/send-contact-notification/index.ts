...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!resendApiKey) {
      console.log("RESEND_API_KEY not configured. Contact message received:", payload);
      return new Response(
        JSON.stringify({ 
          success: true, 
          message: "Contact message saved successfully. Email notifications not configured." 
        }),
        {
          status: 200,
          headers: {
